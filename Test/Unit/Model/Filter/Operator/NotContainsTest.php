...
{
    private NotContains $operator;

    protected function setUp(): void
    {
        $this->operator = new NotContains();
    }

    public function testCodeAndLabel(): void
    {
        $this->assertSame('not_contains', $this->operator->getCode());
        $this->assertSame('does not contain', (string)$this->operator->getLabel());
    }
    ...
    {
        return [
            'absent => true' => ['hello world', 'foo', true],
            'present => false' => ['hello world', 'world', false],
            'empty needle => true' => ['anything', '', true],
            'null haystack => true' => [null, 'x', true],
